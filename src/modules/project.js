...
const saveToLocalStorage = (projectList) => {
  storage("projectList", projectList).save();
};
...
const RenderAllTask = () => {
  ...
  if (!projectList.length) {
    ...
    for (let i = 0; i < projectList.length; i++) {
      ...
      const subProjectName = document.createElement("div");
      ...
      if (!projectList[i].tasks.length) {
        ...
        for (let j = 0; j < projectList[i].tasks.length; j++) {
          ...
          if (priority === "HIGH")
            subListDiv.style.borderLeftColor = "rgb(238, 37, 37)";
          if (priority === "MEDIUM")
            subListDiv.style.borderLeftColor = "rgb(8, 131, 149)";
          if (priority === "LOW")
            subListDiv.style.borderLeftColor = "rgb(255, 229, 105)";
        }
      }
